...
class RAGModelManager:
    ...
    def initialize_model(self, device=None, use_dummy_dataset=True):
        ...
        self.retriever = RagRetriever.from_pretrained(
            ...
            index_name="compressed", # index_name = "compressed" or "exact"
...
def infer_multiple_datasets(rag_manager, configs, save_every_batches=10, batch_size=8):
    ...
    for dataset_type, size, test_only in configs:
        ...
        try:
            ...
            output_path = Path("answers_replicate", f"replicate_{dataset_type}_{size}_answers.json")
            if test_only:
...
def main():
    ...
    rag_manager.initialize_model(device=device, use_dummy_dataset=False)  # Set to True for faster testing
